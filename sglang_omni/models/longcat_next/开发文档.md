...
1. 视觉 tokenizer stage：`SimpleScheduler` + `load_weights_by_prefix(prefix="model.visual_tokenizer.")`
2. 视觉 head stage：`SimpleScheduler` + `load_weights_by_prefix(prefix="visual_head.")`
3. Pipeline 拓扑：`text → visual_tokenizer → visual_head → aggregate`

---

## Phase 3（规划）

1. 音频 tokenizer stage：`SimpleScheduler` + `load_weights_by_prefix(prefix="model.audio_tokenizer.")`
2. 音频 head stage：`SimpleScheduler` + `load_weights_by_prefix(prefix="audio_head.")`
3. 端到端多模态：文本 + 图片 + 语音 输入 → 文本 + 图片 + 语音 输出
